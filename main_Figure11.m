...
global c0 K lambda delta_f Ts Mr Mt
...
N_vec = [16,32,48,64,80]; % correspond to the parameter 'N' in the paper
K1 = K*p1;
...
for i_n = 1:length(N_vec)
    ...
    parfor trial = 1:num_trial
        ...
        RMSE_angle4(i_n, trial) = cal_RMSE(Angle4*pi/180, Angle*pi/180);
        RMSE_range4(i_n, trial) = cal_RMSE(Range4, Range);
        ...
        RMSE_angle1(i_n, trial) = cal_RMSE(Angle1*pi/180, Angle*pi/180);
        RMSE_range1(i_n, trial) = cal_RMSE(Range1, Range);
        RMSE_veloc1(i_n, trial) = cal_RMSE(Veloc1, Veloc);
        ...
        RMSE_angle0(i_n, trial) = cal_RMSE(Angle0*pi/180, Angle*pi/180);
        RMSE_range0(i_n, trial) = cal_RMSE(Range0, Range);
        RMSE_veloc0(i_n, trial) = cal_RMSE(Veloc0, Veloc);
        ...
        RMSE_angle2(i_n, trial) = cal_RMSE(Angle2*pi/180, Angle*pi/180);
        RMSE_range2(i_n, trial) = cal_RMSE(Range2, Range);
        RMSE_veloc2(i_n, trial) = cal_RMSE(Veloc2, Veloc);
        ...
        RMSE_angle2part2(i_n, trial) = cal_RMSE(Angle2part2*pi/180, Angle*pi/180);
        RMSE_range2part2(i_n, trial) = cal_RMSE(Range2part2, Range);
        RMSE_veloc2part2(i_n, trial) = cal_RMSE(Veloc2part2, Veloc);
        ...
        RMSE_angle3(i_n, trial) = cal_RMSE(Angle3*pi/180, Angle*pi/180);
        RMSE_range3(i_n, trial) = cal_RMSE(Range3, Range);
        RMSE_veloc3(i_n, trial) = cal_RMSE(Veloc3, Veloc);

        [rcrb_angle(i_n,trial),rcrb_range(i_n,trial),rcrb_veloc(i_n,trial)] = ...
...
xlabel('\itN', 'FontName', 'Times New Roman','FontSize', 18);
ylabel('RMSE (rad)', 'FontName', 'Times New Roman','FontSize', 18);
set(gca, 'FontName', 'Times New Roman','FontSize', 18, 'XTickLabel',{'16','32','48','64','80'},'XLim',[1,5],'YLim',[0.0004,0.011],'YTick',[1e-3,1e-2]);
...
set(gca, 'FontName', 'Times New Roman','FontSize', 18, 'XTickLabel',{'16','32','48','64','80'},'XLim',[1,5],'YLim',[0.01,1.25],'YTick',[1e-2,1e-1,1e0]);
empty_patch = patch([0 0 1 1], [0 1 1 0], 'w', 'EdgeColor', 'none'); hold on;
...
set(gca, 'FontName', 'Times New Roman','FontSize', 18, 'XTickLabel',{'16','32','48','64','80'},'XLim',[1,5],'YLim',[0.1,1.3],'YTick',[1e-1,1e0]);
